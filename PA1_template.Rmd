...
mRows<-data.frame(interval = StepData[missing,"interval"])
mRows$RowIndex<-missing

# Merge in the missing values.
imputed<-merge(mspi, mRows, by.x="interval", by.y="interval")

# Now we can backfill the missing data in a new data set.
FixedSteps<-StepData

for (i in 1:nrow(imputed)) {
  index<-imputed[i,"RowIndex"]
  FixedSteps[index,"steps"]<-imputed[i,"mean"]
}
